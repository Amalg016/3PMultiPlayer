...
namespace DiasGames.ThirdPersonSystem
{
    public class AnimatorManager : MonoBehaviour
    {
        ...
        {//if (!PV.IsMine) return;
           ...
           // object[] ob = new object[] { newState, transitionDuration.ToString(), layer.ToString() };
            //PV.RPC("SetAnim",RpcTarget.All, ob);
        }
     //  [PunRPC]
        void SetAnim( object[] obj)
        {
            string newState = (string)obj[0];
            float transitionDuration = float.Parse((string)obj[1]);
            int layer = System.Convert.ToInt32((string)obj[2]);
            layerVelocity[layer] = (1 - m_Animator.GetLayerWeight(layer)) / transitionDuration;
            m_Animator.CrossFadeInFixedTime(newState, transitionDuration, layer);
        }
